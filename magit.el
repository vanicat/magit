  ...
  (cond
   ...
   ((looking-at "^diff \\(--git\\|--cc\\)")
    (let (src dst status modes)
      (cond
        ((looking-at "diff --git \\(\"?a/\\(.+\\)\\) \"?b/\\2$")
         (setq dst (substring (magit-decode-git-path (match-string 1)) 2)
               src dst
               status 'modified))
        ((looking-at "diff --cc \\(.+\\)$")
         (setq dst (magit-decode-git-path (match-string 1))
               src dst
               status 'unmerged)))
      (magit-delete-line)
      (while (not (or (eobp) (looking-at magit-diff-headline-re)))
        (cond
          ...
          ((looking-at "^\\(new\\|deleted\\)")
           ...
           (magit-delete-line))
          ((looking-at "^\\(rename\\|copy\\) from \\(.*\\)\n\\1 to \\(.*\\)\n")
           (setq status (intern (match-string 1)))
           (setq src (match-string 2))
           (setq dst (match-string 3))
           (magit-delete-match))
          (t
           (magit-delete-line))))
      (when diffstat
      ...
      (magit-insert-section it
        ...
        (magit-insert-heading
            (propertize (if (memq status '(copy rename))
                            (format "%-10s %s => %s\n" status src dst)
                          (format "%-10s %s\n" status dst))
                      'face 'magit-file-heading))
