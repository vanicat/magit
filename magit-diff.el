...
(defun magit-diff-dwim (&optional args files)
  ...
  (-if-let* ((region (magit-region-sections))
             (fst (car (last region)))
             (last (car region)))
      (if (and (magit-section-match 'commit fst)
               (magit-section-match 'commit last))
          (let* ((last-commit (magit-section-value last))
                 (fst-commit (magit-section-value fst))
                 (range (format "%s..%s" fst-commit last-commit)))
